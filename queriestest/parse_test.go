...
func testClusterMatch(t *testing.T, proxyAuthTenant string, query string, expectedKeys []string, expectedValues []string) { //nolint:unparam
	...
	_, keys, values := queries.ParseAuthorizations(proxyAuthTenant, q)
...
func TestClusterMatches(t *testing.T) {
	proxyAuthTenant := "MyTenant"

	// Test querying metrics without any filters
	// Expect the following authorization permissions:
	testClusterMatch(t, proxyAuthTenant, `cluster:cpu_cores:sum`,
		[]string{
			// - A user with the "Tenant" permission can view all metrics in all tenants.
			"Tenant",
			// - A user with the Tenant-$PROXY_AUTH_TENANT permission can view all metrics for all clusters in the $PROXY_AUTH_TENANT tenant.
			fmt.Sprintf("Tenant-%s", proxyAuthTenant)},
		[]string{"GET", "GET"})

	// Test querying metrics with a filter on a specific nerc-ocp-prod cluster in the $PROXY_AUTH_TENANT tenant.
	// Expect the following authorization permissions:
	testClusterMatch(t, proxyAuthTenant, `cluster:cpu_cores:sum{cluster="nerc-ocp-prod"}`,
		[]string{
			// - A user with the "Tenant" permission can view all metrics in all tenants.
			"Tenant",
			// - A user with the Tenant-$PROXY_AUTH_TENANT permission can view all metrics for all clusters in the $PROXY_AUTH_TENANT tenant.
			fmt.Sprintf("Tenant-%s", proxyAuthTenant),
			// - A user with the Tenant-$PROXY_AUTH_TENANT-AiCluster-nerc-ocp-prod permission can view all metrics for the nerc-ocp-prod cluster in the $PROXY_AUTH_TENANT tenant.
			fmt.Sprintf("Tenant-%s-AiCluster-nerc-ocp-prod", proxyAuthTenant)},
		[]string{"GET", "GET", "GET"})

	// Test querying metrics with a filter on a specific test project in the nerc-ocp-prod cluster in the $PROXY_AUTH_TENANT tenant.
	// Expect the following authorization permissions:
	testClusterMatch(t, proxyAuthTenant, `cluster:cpu_cores:sum{cluster="nerc-ocp-test",namespace="test"}`,
		[]string{
			// - A user with the "Tenant" permission can view all metrics in all tenants.
			"Tenant",
			// - A user with the Tenant-$PROXY_AUTH_TENANT permission can view all metrics for all clusters in the $PROXY_AUTH_TENANT tenant.
			fmt.Sprintf("Tenant-%s", proxyAuthTenant),
			// - A user with the Tenant-$PROXY_AUTH_TENANT-AiCluster-nerc-ocp-test permission can view all metrics for the nerc-ocp-test cluster in the $PROXY_AUTH_TENANT tenant.
			fmt.Sprintf("Tenant-%s-AiCluster-nerc-ocp-test", proxyAuthTenant),
			// - A user with the Tenant-$PROXY_AUTH_TENANT-AiCluster-nerc-ocp-test-AiProject-test permission can view all metrics for the test project in the nerc-ocp-test cluster in the $PROXY_AUTH_TENANT tenant.
			fmt.Sprintf("Tenant-%s-AiCluster-nerc-ocp-test-AiProject-test", proxyAuthTenant)},
		[]string{"GET", "GET", "GET", "GET"})

	// Test querying metrics with a filter on a specific test project witout a filter on a cluster in the $PROXY_AUTH_TENANT tenant.
	// Because no cluster was specified as a filter, this query requires permissions for the $PROXY_AUTH_TENANT tenant, or all tenants.
	// Expect the following authorization permissions:
	testClusterMatch(t, proxyAuthTenant, `cluster:cpu_cores:sum{namespace="test"}`,
		[]string{
			// - A user with the "Tenant" permission can view all metrics in all tenants.
			"Tenant",
			// - A user with the Tenant-$PROXY_AUTH_TENANT permission can view all metrics for all clusters in the $PROXY_AUTH_TENANT tenant.
